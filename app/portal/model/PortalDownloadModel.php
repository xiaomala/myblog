...
class PortalDownloadModel extends Model
{
    ...
    {
        return $this->belongsToMany('PortalTagModel', 'portal_tag_download', 'tag_id', 'download_id');
    }
     ...
     * @return $this
     */
    public function adminAddDownload($data)
    {
     ...
     * @return $this
     */
    public function adminEditDownload($data)
    {
    ...
    public function addTags($keywords, $articleId)
    {
        ...
        if (!empty($keywords))
        {
            ...
            $oldTagIds = Db::name('portal_tag_download')->where('download_id', $articleId)->column('tag_id');
            ...
            foreach ($keywords as $keyword)
            {
                ...
                if (!empty($keyword))
                {
                    ...
                    if (!in_array($tagId, $oldTagIds)) {
                        array_push($data, ['tag_id' => $tagId, 'download_id' => $articleId]);
                    }
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('portal_tag_download')->where('download_id', $articleId)->delete();
            }
            ...
            if (!empty($shouldDeleteTagIds)) {
                Db::name('portal_tag_download')->where(['download_id' => $articleId, 'tag_id' => ['in', $shouldDeleteTagIds]])->delete();
            }

            if (!empty($data)) {
                Db::name('portal_tag_download')->insertAll($data);
            }
        ...
        if (isset($data['id'])) {
            ...
            if ($res) {
                ...
                $recycleData = [
                    'object_id'   => $res['id'],
                    'create_time' => time(),
                    'table_name'  => 'portal_download#page',
                    'name'        => $res['post_title'],
                ...
                $transStatus = false;
                try {
                    Db::name('portal_download')->where(['id' => $id])->update([
                        'delete_time' => time()
                    ]);
            ...
            if ($res) {
                ...
                foreach ($res as $key => $value) {
                    ...
                    $recycleData[$key]['table_name']  = 'portal_download';
                ...
                $transStatus = false;
                try {
                    Db::name('portal_download')->where(['id' => ['in', $ids]])
                        ->update([
                            'delete_time' => time()
                        ]);
