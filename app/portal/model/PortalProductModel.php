...
class PortalProductModel extends Model
{
    ...
    {
        return $this->belongsToMany('PortalProCategoryModel', 'portal_pro_category_product', 'category_id', 'product_id');
    }
    ...
    {
        return $this->belongsToMany('PortalTagModel', 'portal_tag_product', 'tag_id', 'product_id');
    }
    ...
    public function addTags($keywords, $articleId)
    {
        ...
        if (!empty($keywords)) {
            ...
            $oldTagIds = Db::name('portal_tag_product')->where('product_id', $articleId)->column('tag_id');
            ...
            foreach ($keywords as $keyword) {
                ...
                if (!empty($keyword)) {
                    ...
                    if (!in_array($tagId, $oldTagIds)) {
                        array_push($data, ['tag_id' => $tagId, 'product_id' => $articleId]);
                    }
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('portal_tag_product')->where('product_id', $articleId)->delete();
            }
            ...
            if (!empty($shouldDeleteTagIds)) {
                Db::name('portal_tag_product')->where(['product_id' => $articleId, 'tag_id' => ['in', $shouldDeleteTagIds]])->delete();
            }

            if (!empty($data)) {
                Db::name('portal_tag_product')->insertAll($data);
            }
        ...
        if (isset($data['id'])) {
            ...
            if ($res) {
                ...
                $recycleData = [
                    'object_id'   => $res['id'],
                    'create_time' => time(),
                    'table_name'  => 'portal_product#page',
                    'name'        => $res['post_title'],
                ...
                $transStatus = false;
                try {
                    Db::name('portal_product')->where(['id' => $id])->update([
                        'delete_time' => time()
                    ]);
            ...
            if ($res) {
                ...
                foreach ($res as $key => $value) {
                    ...
                    $recycleData[$key]['table_name']  = 'portal_product';
                ...
                $transStatus = false;
                try {
                    Db::name('portal_product')->where(['id' => ['in', $ids]])
                        ->update([
                            'delete_time' => time()
                        ]);
